...
export default function AdminDashboard() {
  ...
  const statCards = [
    { 
      ...
      color: 'text-blue-600',
      bgColor: 'bg-blue-50',
      change: stats.ordersGrowth !== undefined ? `${stats.ordersGrowth > 0 ? '+' : ''}${stats.ordersGrowth}%` : '--',
    ...
    { 
      ...
      change: stats.revenueGrowth !== undefined ? `${stats.revenueGrowth > 0 ? '+' : ''}${stats.revenueGrowth}%` : '--',
      changeType: stats.revenueGrowth > 0 ? 'positive' : stats.revenueGrowth < 0 ? 'negative' : 'neutral',
      description: 'صافي المبيعات خلال الفترة'
    },
    { 
      ...
      change: stats.pendingOrders > 10 ? 'تنبيه' : 'طبيعي',
      changeType: stats.pendingOrders > 10 ? 'negative' : 'positive',
      description: 'طلبات تنتظر المعالجة'
    },
  ];
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          <TabsList className="grid w-full grid-cols-2 md:grid-cols-3 lg:grid-cols-9 h-auto p-1 bg-gray-100">
            ...
            <TabsTrigger value="delivery-fees" className="py-3 data-[state=active]:bg-white">
              <MapPin className="h-4 w-4 ml-2" />
              رسوم التوصيل
            </TabsTrigger>
            <TabsTrigger value="audit-logs" className="py-3 data-[state=active]:bg-white">
